...
def test_reader_notifies_shadow_observer_after_authoritative_delivery(monkeypatch):
    async def scenario():
        order = []
        ...
        def event_observer(event):
            assert isinstance(event, twap.PolymarketTwapEvent)
            order.append("shadow_observer")
            observed.set()
        ...
        assert order[:3] == ["redis", "postgres_queue", "shadow_observer"]

    asyncio.run(scenario())


def test_shadow_observer_failure_does_not_stop_twap_reader(monkeypatch, caplog):
    async def scenario():
        websocket = OneMessageWebSocket(
            json.dumps(current_twap_message(), default=str),
            [],
        ...
        task = asyncio.create_task(
            twap.polymarket_twap_reader_loop(
                ...
                instrument_id=77,
                records=records,
                event_observer=failing_observer,
            )
        )
        while not any(
    ...
    asyncio.run(scenario())
    assert "polymarket_twap_event_observer_failed" in caplog.text
