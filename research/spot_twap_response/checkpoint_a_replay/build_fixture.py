"""Freeze a chronological retained-data hour and independent slot-level oracle.
...
OUTPUT = ROOT / "tests/fixtures/ghost_twap"
...
def reference_rows(events: list[dict], decisions: list[dict]) -> tuple[list[dict], dict]:
    ...
    for decision in decisions:
        ...
        for horizon in HORIZONS:
            ...
            counts = Counter(exact=0, carried=0, future=0, missing=0)
            values: list[Decimal] = []
            if target is None:
                counts["missing"] = 60
            else:
                ...
                for slot_ms in range(target - 62_000, target - 2000, 1000):
                    ...
                    if stamp is None or slot_ms - stamp > 10_000:
                        ...
                        counts["exact" if stamp == slot_ms else "carried"] += 1
...
def main() -> None:
    ...
    manifest = dict(
        ...
        selection="First UTC-aligned hour with at least120s of both-feed export coverage before and after; no quality/error selection",
        ...
        **provenance, **evaluation,
        ...
        limitations=[
            ...
            "Spot receipt milliseconds are multiplied by1000000; this does not create submillisecond measurement precision.",
    ...
    print(json.dumps({"output": str(output_dir), "events": len(events), "decisions": len(decisions),
                      "expected_rows": len(expected), **evaluation}, indent=2))
