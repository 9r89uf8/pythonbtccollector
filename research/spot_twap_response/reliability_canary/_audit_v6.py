"""Independent v6/contract4 campaign audit; no production imports or service access.
...
def verify_reconnect(frozen):
    """Validate bounded retained-history evidence without using engine helpers."""
    policy = frozen['policy']
    require(type(policy.get('spot_reconnect_max_gap_ms')) is int
            and policy['spot_reconnect_max_gap_ms'] == 10000,
            'Reconnect policy mismatch')
    recovery = frozen['spot_reconnect']
    if recovery is None:
        return
    require(set(recovery) == {'gap_ordinal', 'gap_wall_ns', 'gap_monotonic_ns',
            'previous_spot', 'first_post_gap_spot', 'status', 'reason'}, 'Reconnect field contract')
    require(type(recovery['gap_ordinal']) is int and 0 < recovery['gap_ordinal'] <= frozen['gap_count'],
            'Reconnect ordinal mismatch')
    require(recovery['status'] in ('waiting', 'retained', 'cleared') and
            isinstance(recovery['reason'], str) and 0 < len(recovery['reason']) <= 265,
            'Reconnect status/reason invalid')
    for name, decision in (('gap_wall_ns', 'decision_wall_ns'), ('gap_monotonic_ns', 'decision_monotonic_ns')):
        require(recovery[name] is None or 0 <= integer(recovery[name]) <= integer(frozen[decision]),
                'Reconnect clock after decision')
    previous, first = recovery['previous_spot'], recovery['first_post_gap_spot']
    for event in (previous, first):
        if event is not None:
            event_check(event, 'spot')
            require(integer(event['received_wall_ns']) <= integer(frozen['decision_wall_ns']) and
                    integer(event['received_monotonic_ns']) <= integer(frozen['decision_monotonic_ns']),
                    'Reconnect event after decision')
    if recovery['status'] == 'cleared':
        return
    require(previous is not None and recovery['gap_wall_ns'] is not None and
            recovery['gap_monotonic_ns'] is not None, 'Retention missing prior/gap clocks')
    require(any(g['feed'] == 'spot' and g['reason'] == 'connection_end' and
                g['ordinal'] == recovery['gap_ordinal'] for g in frozen['last_gaps']),
            'Retention has no matching transport gap')
    wall, mono = integer(recovery['gap_wall_ns']), integer(recovery['gap_monotonic_ns'])
    ages = (wall - previous['source_timestamp_ms'] * MS,
            wall - integer(previous['received_wall_ns']), mono - integer(previous['received_monotonic_ns']))
    require(previous['source_timestamp_ms'] * MS <= integer(previous['received_wall_ns']) and
            min(ages) >= 0 and ages[0] <= 5000 * MS and max(ages[1:]) <= 3000 * MS,
            'Pre-gap spot was not fresh')
    if recovery['status'] == 'waiting':
        require(first is None and frozen['current_spot'] is None and
                all(f['price'] is None for f in frozen['forecasts']), 'Waiting reconnect exposed prices')
        return
    require(first is not None and frozen['current_spot'] is not None and
            recovery['reason'] == 'bounded_carry_estimates', 'Retained reconnect missing resume evidence')
    deltas = ((first['source_timestamp_ms'] - previous['source_timestamp_ms']) * MS,
              integer(first['received_wall_ns']) - integer(previous['received_wall_ns']),
              integer(first['received_monotonic_ns']) - integer(previous['received_monotonic_ns']))
    require(deltas[0] > 0 and min(deltas[1:]) >= 0 and max(deltas) <= 10000 * MS and
            integer(first['received_wall_ns']) >= wall and integer(first['received_monotonic_ns']) >= mono and
            first['sequence'] > previous['sequence'] and frozen['current_spot']['sequence'] >= first['sequence'] and
            0 <= integer(first['received_wall_ns']) - first['source_timestamp_ms'] * MS <= 5000 * MS,
            'Retained reconnect exceeded bounded resume contract')


def verify_calculation(frozen, state):
    wall, mono = integer(frozen['decision_wall_ns']), integer(frozen['decision_monotonic_ns'])
    require(frozen['contract_version'] == 4 and frozen['runtime_version'] == 'ghost-canary-v6'
...
def verify_publication(frozen, state):
    ...
    for field in ('run_id', 'decision_id', 'decision_wall_ns', 'decision_monotonic_ns', 'current_spot', 'current_twap',
                  'contract_version', 'policy', 'valid_until_wall_ns', 'spot_reconnect'):
        require(payload[field] == frozen[field], 'Payload/frozen mismatch: ' + field)
    require(payload['runtime_version'] == 'ghost-canary-v6', 'Wrong wire runtime version')
    ...
    require(isinstance(selection, dict) and type(selection.get('version')) is int and selection['version'] == 1,
            'Unknown or missing v6 publication mask')
    ...
    if pub['status'] in ('acknowledged', 'uncertain'):
        ...
        wall, mono = integer(frozen['decision_wall_ns']), integer(frozen['decision_monotonic_ns'])
        deadline = integer(frozen['valid_until_wall_ns'])
        remaining = min(deadline - checked_wall, deadline - wall - (checked_mono - mono))
        require(remaining // MS > 0, 'Actual publication attempted after usable expiry')
        require(checked_wall < (frozen['runtime_policy']['canary_start_ms'] + HOUR_MS) * MS,
                'Actual publication attempted after campaign end')
...
class Audit:
    def __init__(self, campaign_start_ms):
        ...
        self.reconnects, self.operational_gaps = {}, {}

    def row(self, row, frozen, state, verified=None):
        if verified is None:
            events = verify_calculation(frozen, state)
            eligible, payload = verify_publication(frozen, state)
        else:
            events, eligible, payload = verified
        self.counts.update('global_reason_' + reason for reason in frozen['reasons'])
        reconnect = frozen.get('spot_reconnect')
        self.counts['reconnect_state_' + ('none' if reconnect is None else reconnect['status'])] += 1
        if reconnect is not None:
            self.reconnects[reconnect['gap_ordinal']] = reconnect
        for gap in frozen.get('operational_gaps', []):
            self.operational_gaps[sha256(json.dumps(gap, sort_keys=True).encode()).hexdigest()] = gap
        wall, mono = integer(frozen['decision_wall_ns']), integer(frozen['decision_monotonic_ns'])
        ...
        for forecast in frozen['forecasts']:
            ...
            counts.update('unavailable_reason_' + reason for reason in forecast['reasons'])
        ...
        return events, eligible, payload

    def summary(self):
        return dict(status='accepted', campaign_start_ms=self.start, campaign_end_ms_exclusive=self.start + HOUR_MS,
                    runtime_version='ghost-canary-v6', contract_version=4, publication_eligibility_policy=POLICY,
                    counts=dict(self.counts), publication_statuses=dict(self.publication), run_rows=dict(self.runs),
                    run_details=self.run_details, reconnect_episodes=list(self.reconnects.values()),
                    saved_operational_gaps=list(self.operational_gaps.values()),
