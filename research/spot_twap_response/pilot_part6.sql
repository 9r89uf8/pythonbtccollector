-- Spot-to-TWAP response pilot, part 6 (read-only, research only). 2026-09-13.
-- Receipt-clock ghost TWAP: at wall-clock instant tau, use ONLY data received by tau.
--   latest TWAP  = the TWAP event with the newest receipt <= tau (stamp w, price = persistence baseline)
--   slot value   = for each window slot u, the newest Chainlink spot sample stamped <= u AND received <= tau
--                  (this single rule carries the last received value through silent seconds and fills
--                   future slots with the latest received spot)
--   ghost(h)     = mean of slot values over the window of the TWAP stamped w+h, i.e. [w+h-62 s, w+h-3 s]
--   target       = the TWAP event stamped w+h; graded on price and on its side against the Price to Beat
--                  of the market containing w+h; its receipt minus tau is the measured arrival delay.
-- Instants: every 60 s of the pilot week. Horizons 5, 10 and 30 s. price_samples keeps the newest
-- same-second update's receipt time, so a slot's receipt is a retained-history approximation.
-- Week: 1788220800000 = 2026-09-01T00:00Z .. 1788825600000 = 2026-09-08T00:00Z.
-- Run: sudo -u postgres psql -X -At -v ON_ERROR_STOP=1 -d price_collector -f pilot_part6.sql
