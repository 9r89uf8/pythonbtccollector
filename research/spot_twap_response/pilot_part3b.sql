-- Spot-to-TWAP response pilot, part 3b (read-only, research only). 2026-09-13.
-- Section 13: the week-long projection pilot rerun with the carry-forward input policy that
-- pilot_part3.sql section 11 supports. Per-market formulation: for each resolved market the 60
-- constituent seconds [E-62 s, E-3 s] are filled with the last retained Chainlink spot at or before
-- each second (index lookups), so no market is excluded for missing samples and no whole-week series is built.
-- Alignment: newest constituent stamp = t - 3 s (pilot_part2 section 8). Provider clocks throughout.
-- Week: 1788220800000 = 2026-09-01T00:00Z .. 1788825600000 = 2026-09-08T00:00Z.
-- Run: sudo -u postgres psql -X -At -v ON_ERROR_STOP=1 -d price_collector -f pilot_part3b.sql
