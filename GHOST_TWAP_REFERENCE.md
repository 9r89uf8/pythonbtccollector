...
The full-market condition recorder was installed September 23, 2026 at 15:20
UTC: feature commit `65966e4`, migration fix `52d5596`, local dashboard
`2314873`. The new schedule constraint finished validation at 15:23:20 UTC.
The first production checks confirmed schema-4 recorded observations with up
to 299,272 ms remaining, fresh schema-2 history, and no settlement runtime
fault. The older deployment records below describe their original releases.

The first migration attempt exceeded its 30-second deadline while scanning
existing rows and rolled back. The corrected
[full-market migration procedure](README.md#full-market-historical-win-rate-migration)
installs the check with `NOT VALID`, commits, restarts collection, then validates
retained rows separately. New inserts and updates are checked throughout;
PostgreSQL validation was confirmed to allow concurrent writes.

An existing capacity pause predated this release: the last legacy observation
was September 22 at 23:09 UTC, and settlement relations used 520,126,464 bytes,
32 KiB above the strict 496 MiB admission threshold. Ordinary bounded VACUUM
...
The local panel was checked with live spot/TWAP before the final minute. Its
Node tests also consume synthetic output generated by the real backend functions
(30 invented markets, 24 wins and six losses), verifying the combined 80% display,
indexed cohort references and coverage fields. This fixture is not live history.
