...
The follow-up is deliberately small. The offline
[`shadow_signal_lag_test.py`](price_collector/shadow_signal_lag_test.py) wrapper
reuses the existing raw replay for one calibration window and, only after a
shorter lag is selected, one later holdout window against the 3.0-second
fixed-settings reference. That reference is not a reconstruction of the
deployed incumbent. The helper writes a descriptive JSON result and cannot
promote or deploy a model; it also does not prove that its ranges were
preregistered. The fixed inputs and command are in
[`CHAINLINK_V4_PRETEST_FIX_PLAN.md`](CHAINLINK_V4_PRETEST_FIX_PLAN.md).
