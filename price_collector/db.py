...
TWAP_SHADOW_HORIZONS_SECONDS = (1, 3, 5, 10)


async def fetch_twap_shadow_preload(
    pool: asyncpg.Pool,
    *,
    cutoff_received_ms: int,
    lookback_ms: int,
) -> dict[str, list[dict[str, Any]]]:
    """Load only observations already available at the startup cutoff."""
    ...
    async with pool.acquire() as connection:
        source_rows = await connection.fetch(
            ...
            WITH source_observations AS (
                SELECT 'futures'::TEXT AS source, source_ms, received_ms, price
                FROM (
                    SELECT DISTINCT ON (snapshot.futures_last_price_time_ms)
                        snapshot.futures_last_price_time_ms AS source_ms,
                        snapshot.received_ms AS received_ms,
                        snapshot.futures_last_price AS price
                    FROM binance_futures_snapshots snapshot
                    WHERE snapshot.symbol = 'BTCUSDT'
                      AND snapshot.futures_last_price IS NOT NULL
                      AND snapshot.sample_second_ms >= $1::BIGINT - 1000
                      AND snapshot.futures_last_price_time_ms >= $1::BIGINT
                      AND snapshot.received_ms <= $2::BIGINT
                    ORDER BY
                        snapshot.futures_last_price_time_ms,
                        snapshot.received_ms ASC
                ) futures_history

                UNION ALL

                SELECT 'binance_spot'::TEXT AS source, source_ms, received_ms, price
                FROM (
                    SELECT DISTINCT ON (sample.provider_event_ms)
                        sample.provider_event_ms AS source_ms,
                        sample.received_ms AS received_ms,
                        sample.price AS price
                    FROM price_samples sample
                    JOIN instruments instrument
                      ON instrument.instrument_id = sample.instrument_id
                    JOIN providers provider
                      ON provider.provider_id = instrument.provider_id
                    WHERE provider.provider_code = 'binance_spot'
                      AND instrument.symbol = 'BTCUSDT'
                      AND sample.sample_second_ms >= $1::BIGINT - 1000
                      AND sample.provider_event_ms >= $1::BIGINT
                      AND sample.received_ms <= $2::BIGINT
                    ORDER BY sample.provider_event_ms, sample.received_ms ASC
                ) binance_history

                UNION ALL

                SELECT 'chainlink_spot'::TEXT AS source, source_ms, received_ms, price
                FROM (
                    SELECT DISTINCT ON (sample.provider_event_ms)
                        sample.provider_event_ms AS source_ms,
                        sample.received_ms AS received_ms,
                    ...
                    JOIN providers provider
                      ON provider.provider_id = instrument.provider_id
                    WHERE provider.provider_code = 'polymarket_chainlink_rtds'
            ...
            SELECT source, source_ms, received_ms, price
            FROM source_observations
            ORDER BY received_ms ASC, source ASC, source_ms ASC
            """,
            source_start_ms,
            cutoff_received_ms,
        )
        actual_rows = await connection.fetch(
            """
            SELECT
                event.sample_second_ms AS provider_event_ms,
    ...
    return {
        ...
        "actuals": [dict(row) for row in actual_rows],
    }


async def persist_twap_shadow_prediction_batch(
    pool: asyncpg.Pool,
    batch: Any,
) -> int:
    """Persist the first forecast retained for each target/horizon."""

    model_version = int(batch.model_version)
    generated_ms = int(batch.generated_ms)
    forecasts = [
        forecast for forecast in batch.forecasts if forecast.value is not None
    ]
    if not forecasts:
        return 0
    ...
    async with pool.acquire() as connection:
        async with connection.transaction():
            for forecast in forecasts:
                ...
                if horizon_seconds not in TWAP_SHADOW_HORIZONS_SECONDS:
                    raise ValueError("unsupported TWAP shadow horizon")
                target_second_ms = int(forecast.target_second_ms)
                window = market_for_sample_second(target_second_ms)
                await _ensure_market_window(connection, window)
                prefix = f"h{horizon_seconds}"
                await connection.execute(
                    f"""
                    INSERT INTO chainlink_twap_shadow_predictions (
                        target_second_ms,
                        model_version,
                        market_id,
                        {prefix}_price,
                        {prefix}_generated_ms,
                        {prefix}_known_fraction,
                        {prefix}_source_count,
                        {prefix}_estimated_error_bps
                    )
                    VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
                    ON CONFLICT (target_second_ms, model_version)
                    DO UPDATE SET
                        {prefix}_price = EXCLUDED.{prefix}_price,
                        {prefix}_generated_ms = EXCLUDED.{prefix}_generated_ms,
                        {prefix}_known_fraction = EXCLUDED.{prefix}_known_fraction,
                        {prefix}_source_count = EXCLUDED.{prefix}_source_count,
                        {prefix}_estimated_error_bps =
                            EXCLUDED.{prefix}_estimated_error_bps,
                        updated_at = now()
                    WHERE chainlink_twap_shadow_predictions.{prefix}_price
                          IS NULL
                    """,
                    target_second_ms,
                    model_version,
                    ...
                    forecast.value,
                    generated_ms,
                    forecast.known_fraction,
                    int(forecast.source_count),
                    forecast.estimated_error_bps,
                )
    return len(forecasts)


async def delete_expired_twap_shadow_predictions(
    pool: asyncpg.Pool,
    *,
    cutoff_target_ms: int,
    batch_size: int = 5_000,
) -> int:
    """Bound optional shadow retention work so it cannot monopolize PostgreSQL."""

    if cutoff_target_ms < 0:
        raise ValueError("cutoff_target_ms must be non-negative")
    if batch_size <= 0:
    ...
    async with pool.acquire() as connection:
        status = await connection.execute(
            """
            WITH doomed AS (
                SELECT target_second_ms, model_version
                FROM chainlink_twap_shadow_predictions
                WHERE target_second_ms < $1::BIGINT
                ORDER BY target_second_ms ASC, model_version ASC
                LIMIT $2::INTEGER
            )
            DELETE FROM chainlink_twap_shadow_predictions prediction
            USING doomed
            WHERE prediction.target_second_ms = doomed.target_second_ms
              AND prediction.model_version = doomed.model_version
            """,
            cutoff_target_ms,
            batch_size,
        )
...
async def fetch_twap_shadow_market_history(
    pool: asyncpg.Pool,
    *,
    market_id: int,
    model_version: int,
    ...
    async with pool.acquire() as connection:
        market = await connection.fetchrow(
            """
            SELECT market_id, market_start_ms, market_end_ms
            FROM market_windows
        ...
        rows = await connection.fetch(
            ...
            WITH target_grid AS (
                SELECT generate_series(
                    ...
                    $2::BIGINT - 1000,
                    1000
                )::BIGINT AS target_second_ms
            ),
            actual_twap AS (
                SELECT DISTINCT ON (event.sample_second_ms)
                ...
                ORDER BY
                    event.sample_second_ms,
                    event.provider_event_ms DESC,
                    event.received_wall_ns DESC,
                    event.connection_id DESC,
                    event.receive_sequence DESC
            )
            SELECT
                target.target_second_ms,
                prediction.model_version,
                prediction.h1_price,
                prediction.h1_generated_ms,
                prediction.h1_known_fraction,
                prediction.h1_source_count,
                prediction.h1_estimated_error_bps,
                prediction.h3_price,
                prediction.h3_generated_ms,
                prediction.h3_known_fraction,
                prediction.h3_source_count,
                prediction.h3_estimated_error_bps,
                prediction.h5_price,
                prediction.h5_generated_ms,
                prediction.h5_known_fraction,
                prediction.h5_source_count,
                prediction.h5_estimated_error_bps,
                prediction.h10_price,
                prediction.h10_generated_ms,
                prediction.h10_known_fraction,
                prediction.h10_source_count,
                prediction.h10_estimated_error_bps,
                actual.price AS actual_price,
                actual.provider_event_ms AS actual_provider_event_ms,
                actual.received_ms AS actual_received_ms
            FROM target_grid target
            LEFT JOIN chainlink_twap_shadow_predictions prediction
              ON prediction.target_second_ms = target.target_second_ms
             AND prediction.model_version = $3::SMALLINT
            LEFT JOIN actual_twap actual
              ON actual.sample_second_ms = target.target_second_ms
            ORDER BY target.target_second_ms ASC
            """,
            int(market["market_start_ms"]),
